...
public class parser {
	...
	//since we should only be examining one token ahead of where we are in the scentence, this will be used
	//to keep track of where we are 
	static int token = 0;
	
	static List IDs = new List(); //identifier table 
	static List FunName = new List();
	...
	static String Global = "12"; static String declarations = "13";
	
	//operator or boolean tokens
	static String equal = "35"; static String ge = "38"; static String ne = "41"; static String mul = "44";
	static String le = "36"; static String gt = "39"; static String plus = "42"; static String div = "45";
	static String lt = " 37"; static String eq = "40"; static String minus = "43";
	
	//Integer token
	static String integer = "47";
	
	//float token
	static String Float = "48";
	
	//String token
	static String literal = "49";
	
	//token for IDs
	static String Ident = "50";
	...
	public static boolean checkGrammar(){
			...
			System.out.println("<forward_refs>");
			if(tokens.getItem(token).equals(forward))
			{
				...
				if(tokens.getItem(token).equals(references))
				{
			...
			{
				System.out.println("Error on line: " + lines.getItem(token) + tokens.getItem(token));
				return false;
			}
	...
	public static boolean checkSymbol() { //checking symbols 
		...
		if(!checkIdentifier(lexemes.getItem(token)))
		{
			if(tokens.getItem(token).equals(Ident))
			{
				...
				token++;
				if(tokens.getItem(token).equals(integer) ||
						tokens.getItem(token).equals(Float) ||
						tokens.getItem(token).equals(literal))//checking constant
				{
	...
	public static boolean checkParamets() {
	...
	public static boolean checkHeader() {
		...
		if(checkFunName(lexemes.getItem(token)))
		{
			return true;
		}
		else
		{
			if(tokens.getItem(token).equals(Ident))// make sure not a keyword 
			{
				System.out.println("IDENTIFIER");
				IDs.add(lexemes.getItem(token));
				token++;
	...
	public static boolean checkStatement() {
		...
		else if(tokens.getItem(token).equals(set))//set
		{
			...
			token++;
			if(!checkIdentifier(lexemes.getItem(token))) //if not in identifier table, will add 
			{
				System.out.println("IDENTIFIER");
				int addition = token;
	...
	public static boolean checkArithmatic() {
		...
		if(checkPrimary()) //checks to make sure it starts with a number / id
		{
			if(tokens.getItem(token).equals(plus) || tokens.getItem(token).equals(minus)) // checks +/-
			{
				...
				{
				if(checkMulexp())//sends to *|/ method
					return true;
				else
					return false;
				}
	...
	public static boolean checkConst(){
		...
		if(tokens.getItem(token).equals(Float) || tokens.getItem(token).equals(integer))
			return true;
		else
			return false;
	...
	public static void main(String[] args)throws Exception{
		...
		scanner list = new scanner();
		List tokenized = list.getList();
		int size;
		...
		for(int i = 0; i < tokenized.getItemCount(); i++) {
            ...
            String[] split = str.split(" ");
            								//splits the list from the lexical analyzer into the lists we will use here
            lines.add(split[0]);
            tokens.add(split[1]);
            lexemes.add(split[2]);
		}
		for(int i = 0; i < lexemes.getItemCount(); i++) {
            
           System.out.println(lines.getItem(i) + " " + tokens.getItem(i) + " " + lexemes.getItem(i));
		}
