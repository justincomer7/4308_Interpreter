...
public class parser {
	...
	static List FunName = new List(); //used to list function names
							...
							if(checkStatement()) //check statement list 
							{
								if(tokens.getItem(token).equals(endif)) {//endif
									...
									if(!checkStatement() && !tokens.getItem(token).equals(endfun)){
										if(!checkStatement()) //check following statements
											return false;
										else
											return true;
										}
			...
			if(checkIdentifier(lexemes.getItem(token))) //makes sure it was declared 
			{
				...
				if(tokens.getItem(token).equals(equal)) // = 
				{
					...
					if(checkArithmatic())
					{
						...
						if(tokens.getItem(token).equals(endfun) || tokens.getItem(token).equals(until) || tokens.getItem(token).equals(endwhile) ||
						tokens.getItem(token).equals(Else) || tokens.getItem(token).equals(endif))
							return true; //checking to see if end of statment list
						else if(checkStatement()) //check following statements
							return true;
						else
							return false;
						}
